...
- Does **not** require a plugin; same-module support is automatic, while cross-module reverse
  relationships use a bounded module list in the Pyrefly configuration
...
Pyrefly infers reverse `ForeignKey`, `OneToOneField`, and `ManyToManyField` relationships. When
both models are declared in the same module, no configuration is needed:
...
For models declared in different modules, list the modules that contain model definitions in
`pyrefly.toml`:

```toml
django-model-modules = [
  "accounts.models",
  "articles.models",
]
```

In `pyproject.toml`, put the option under `[tool.pyrefly]`. Each entry is an importable module name,
not a filesystem path or glob. Listing the modules lets Pyrefly load them lazily and reliably
invalidate reverse relationships as code changes without scanning the entire project.
