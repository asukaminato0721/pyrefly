...
Pyrefly warns when a `match` over a closed subject type, such as an enum, literal
union, `None`, `bool`, or non-subclassable class, may fall through without matching
a case. Add the missing cases or a default arm.

Non-exhaustive matches over open subject types are reported under a sub-kind,
[non-exhaustive-match-open-type](#non-exhaustive-match-open-type).
...
Enable this error kind to have Pyrefly report when a `match` over an open
subject type such as `int`, `str`, or `object` is non-exhaustive.

This is a sub-kind of [non-exhaustive-match](#non-exhaustive-match), so
configuring or suppressing the parent kind also applies to this diagnostic.
...
Reports repeated iteration over the same generator variable. Membership tests,
`for` loops, eager comprehensions, and container construction consume generator
values. A later iteration can therefore produce fewer values, or none at all.

```python
values = (i for i in range(3))
assert 4 not in values
assert 1 in values  # reused-generator: the previous search exhausted values
```

Materialize the values in a list if they need to be traversed repeatedly, or
create a fresh generator before each traversal. Reassigning the variable resets
the check. Lists and other reusable containers do not trigger this diagnostic.

This check tracks variables within a scope, including branches. It does not
track aliases or consumption inside called functions. Creating a generator
expression does not itself consume its input. A previous iteration might stop
early, so this diagnostic indicates possible exhaustion, not guaranteed
exhaustion. Suppress `reused-generator` when resuming iteration is intentional.
...
An instance attribute is declared with a type annotation in the class body but is never assigned a value there or in a recognized method such as `__init__`, so reading it raises `AttributeError` at runtime. Enable this check to require that annotated instance attributes are initialized. This mirrors pyright's `reportUninitializedInstanceVariable`. `ClassVar` and `Final` members, `NamedTuple` and `Protocol` classes, and dataclasses whose synthesized `__init__` initializes their fields are not reported. A plain `@dataclass(init=False)` synthesizes no `__init__`, so its annotation-only fields are reported — a case pyright misses.

```python
class C:
    x: int  # error: Instance attribute `x` is declared but never initialized
