...
A function overload is unreachable when an earlier overload accepts every input that it accepts.
Order narrower overloads before broader overloads so each signature can be selected.
...
def f(x: object) -> object: ...
@overload
def f(x: int) -> int: ...  # error, the first overload already accepts every int
def f(x: object) -> object: ...
```
