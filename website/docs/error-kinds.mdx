...
You may see `@_` in a revealed type (and in hovers). This is a placeholder for a synthetic type
variable that has not been pinned yet, usually because the type comes from an empty container
literal like `[]` or `{}` and hasn't been constrained by a later use.

```python
from typing import reveal_type

x = []
reveal_type(x)  # revealed type: list[@_]
x.append(1)
