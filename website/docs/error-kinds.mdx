...
- **warn**: the diagnostic shows up yellow in the IDE and does not cause a nonzero exit in the CLI
...
This is a type of [bad override](#bad-override) but has a separate error kind so that it can be selectively enabled when you want to enforce stable parameter names across overrides.
...
This diagnostic is also used for multiple inheritance when two inherited methods differ only by positional parameter name. The default severity is `ignore`; set it to `error` if you want Pyrefly to flag these mismatches:

```toml
[tool.pyrefly]
errors = { bad-param-name-override = "error" }
```
