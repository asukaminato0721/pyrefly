...
This error is raised when `TYPE_CHECKING` is assigned a value other than
`False`, or when its annotation cannot accept `bool`. Type checkers treat
`TYPE_CHECKING` as true, so a local runtime definition must be exactly `False`
and any annotation must be compatible with both runtime and type-checking uses.

```python
TYPE_CHECKING: str = ""  # Invalid `TYPE_CHECKING` annotation [invalid-type-checking-constant]
TYPE_CHECKING = True  # `TYPE_CHECKING` must be assigned `False` [invalid-type-checking-constant]
```
