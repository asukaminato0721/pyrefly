...
Lists modules that declare Django models. Pyrefly uses these modules to discover reverse
relationships whose source and target models are defined in different modules. Relationships
between models in the same module are discovered automatically.
...
List every module that directly contains model definitions with `ForeignKey`, `OneToOneField`, or
`ManyToManyField` relationships. Pyrefly loads only the listed modules when it needs cross-module
reverse relationships; it does not scan all project or currently loaded modules.
...
site-package-path = []
# no modules are searched for cross-module Django reverse relationships
django-model-modules = []
