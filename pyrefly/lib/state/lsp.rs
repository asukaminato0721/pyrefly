...
impl<'a> Transaction<'a> {
    ...
    fn documentation_for_completion_export(
        ...
        self.search_exports_exact(symbol, None)
            .ok()?
            .into_iter()
            .find(|(handle, _)| handle.module() == module_name)
            .and_then(|(handle, export)| self.get_documentation_from_export(Some((handle, export))))
    }
        ...
        match data {
            CompletionResolveData::Export {
                    ...
                    self.documentation_for_completion_export(module_name, &symbol)
                {
                    ...
                    && let Some(documentation) = filesystem_docstring(range, path_str)
                {
                    item.documentation = Some(documentation);
                }
            }
