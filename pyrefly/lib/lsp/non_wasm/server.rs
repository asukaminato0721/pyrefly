...
impl Server {
    ...
    /// Find the handle the type checker analyzes `path` under, so `(uri, range)`
    /// queries resolve for any analyzable file rather than only open documents.
    ///
    /// The transaction manager preserves answers produced by earlier queries as
    /// long as the committed state has not changed. For closed files we reuse the
    /// handle the file was already analyzed under — an imported dependency's
    /// filesystem handle, or a bundled stdlib stub's `BundledTypeshed` handle
    /// whose `SysInfo` we can't reconstruct here, hence the by-path lookup. A file
    /// that isn't analyzed yet falls back to a fresh filesystem handle read from
    /// disk. `ensure_answers` performs a full solve only when this state snapshot
    /// does not already contain the retained bindings and answers the query reads.
    fn query_handle(&self, transaction: &mut Transaction, path: &Path) -> Handle {
        ...
        } else if transaction.get_module_info(&fs_handle).is_some() {
            fs_handle
        } else {
            transaction
                .handles()
        ...
        transaction.ensure_answers(&handle, None);
        handle
    }

    /// Open `uri` at `(line, character)`, returning its handle and in-file position.
    fn open_at_position(
        &self,
        transaction: &mut Transaction,
        uri: &str,
...
impl TspInterface for Server {
    ...
    fn computed_type_at_range(
        ...
        let ty = transaction.get_computed_type_at_range(&handle, TextRange::new(start, end))?;
        Some(self.convert_type_in_transaction(transaction, &handle, &ty))
    }
