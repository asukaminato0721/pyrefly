    ...
    for msg in &connection.receiver {
        match msg {
            ...
            Message::Notification(x) => {
                ...
                } else if x.method == "pyrefly/completionItemSelected" {
                    match serde_json::from_value::<CompletionItemSelectedParams>(x.params.clone()) {
                        Ok(params) => lsp_queue.send(LspEvent::CompletionItemSelected(params)),
                        Err(err) => {
                            info!("Invalid params for pyrefly/completionItemSelected: {err}");
                            Ok(())
                        }
                    }
...
impl Server {
    ...
    fn process_event<'a>(
        ...
        match event {
            ...
            LspEvent::CompletionItemSelected(params) => {
                self.completion_item_selected(params);
            }
